...
BASELINES = {
    ...
    "laion2b",
    "x_time",
    "x_time_intersect_clip_score",
    "text_length_as_x_time_intersect_clip_score"
}
...
def check_args(args):
    ...
    # x_time checks
    if "x_time" in args.name:
        if args.xtime_fraction is None and args.xtime_threshold is None:
            raise ValueError(
                "--xtime_fraction or --xtime_threshold must be specified for x_time baselines"
            )
        if args.xtime_fraction is not None and args.xtime_threshold is not None:
            raise ValueError(
                "specify either --xtime_fraction or --xtime_threshold for x_time baselines but not both"
            )
        if args.xtime_arch is None:
            raise ValueError(f"specify architecture {MERU_ARCH}, for x_time baselines")
    if args.xtime_fraction is not None and not ("x_time" in args.name):
        raise ValueError("--xtime_fraction value only used for x_time baselines")
    if args.xtime_threshold is not None and not ("x_time" in args.name):
        raise ValueError("--xtime_threshold value only used for x_time baselines")
    if args.xtime_arch is not None and not ("x_time" in args.name):
        raise ValueError("--xtime_arch value only used for x_time baselines")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="A threshold to apply on a MERU x_time score , after the filtering of CLIP score!(e.g., a value of 0.25 will only keep examples with x_time over 0.25)",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="a fraction of metadata to keep according to MERU x_time score , after the filtering of CLIP score!(e.g., a value of 0.25 will keep the top 25 percent of examples in the pool by MERU x_time score)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        choices=list(MERU_ARCH),
        help="kinds of features (l, b, s) on which to run the MERU x_time filter",
    )
