...
def load_uids_with_x_time_helper(
    fs_url: Tuple[Any, str], xtime_key: str, xtime_threshold: float, 
    key: Union[str, None], threshold: Union[float, None], gcld3_en_filter: bool
) -> np.ndarray:
    """helper to load parquet metadata with a threshold applied to a column
    ...
    if gcld3_en_filter:
        ...
        df = pd.read_parquet(url, columns=["uid", xtime_key, key], filesystem=fs)
    ...
    if key is not None:
        return np.array(
            [
                (int(uid[:16], 16), int(uid[16:32], 16))
                for uid in df[(df[xtime_key] >= xtime_threshold) & (df[key] >= threshold)]["uid"].values
            ],
            np.dtype("u8,u8"),
        )
    return np.array(
        [
            (int(uid[:16], 16), int(uid[16:32], 16))
            for uid in df[df[xtime_key] >= xtime_threshold]["uid"].values
        ],
        np.dtype("u8,u8"),
    )

def load_uids_with_text_length_helper(
    fs_url: Tuple[Any, str], text_key: str, text_threshold: float, 
    key: Union[str, None], threshold: Union[float, None], gcld3_en_filter: bool
) -> np.ndarray:
    """helper to load parquet metadata with a threshold applied to a column
    ...
    if gcld3_en_filter:
        ...
        df = pd.read_parquet(url, columns=["uid", text_key, key], filesystem=fs)


    if key is not None: # using CLIP score as filter first
        df = df[df[key] >= threshold]

    df[text_key+"_length"] = df[text_key].apply(lambda x: len(x))
    
    return np.array(
        [
            (int(uid[:16], 16), int(uid[16:32], 16))
            for uid in df[df[text_key+"_length"] >= text_threshold]["uid"].values
        ],
        np.dtype("u8,u8"),
    )
...
def get_xtime_threshold(
    metadata_dir_path: str, xtime_key: str, xtime_fraction: float, key: str, threshold: float, num_workers: int
) -> float:
    """compute a threshold given a collection of metadata, a key, and a target fraction of the pool to keep
    ...
    """
    print("loading all metadata for threshold computation")
    df = load_metadata(metadata_dir_path, num_workers=num_workers, columns=[xtime_key, key])
    filtered_df = df[df[key] >= threshold]
    n = int(len(filtered_df) * xtime_fraction)
    xtime_threshold = -np.sort(-filtered_df[xtime_key].values)[n-1]
    print(threshold, xtime_threshold)
    return xtime_threshold

def get_text_threshold(
    metadata_dir_path: str, text_key: str, text_fraction: float, key: str, threshold: float, num_workers: int
) -> float:
    """compute a threshold given a collection of metadata, a key, and a target fraction of the pool to keep
    ...
    """
    print("loading all metadata for threshold computation")
    df = load_metadata(metadata_dir_path, num_workers=num_workers, columns=[text_key, key])
    filtered_df = df[df[key] >= threshold]
    filtered_df[text_key+"_length"] = filtered_df[text_key].apply(lambda x: len(x))
    n = int(len(filtered_df) * text_fraction)
    text_threshold = -np.sort(-filtered_df[text_key+"_length"].values)[n-1]
    print(threshold, text_threshold)
...
def load_uids_with_x_time(
    metadata_dir_path: str,
    xtime_arch: str,
    xtime_threshold: Union[float, None],
    xtime_fraction: Union[float, None],
    num_workers: int,
    arch: Union[str, None] = None,
    threshold: Union[float, None] = None,
    fraction: Union[float, None] = None,
    ...
    """using MERU x_time as the filter

    Args:
        metadata_dir_path (str): directory where metadata is stored
        xtime_arch (str): kind of features for meru filtering. 
        xtime_threshold (Union[float, None]): threshold to apply to meru features. 
        xtime_fraction (Union[float, None]): top k fraction to apply to meru features. 
        num_workers (int): number of cpu works used to load metadata to compute threshold. 
        arch (Union[str, None], optional): kind of features for clip filtering. Defaults to None.
        threshold (Union[float, None], optional): threshold to apply to clip features. Defaults to None.
        fraction (Union[float, None], optional): top k fraction to apply to clip features. Defaults to None.
    ...
    xtime_key = xtime_arch + "_xtime"
    if xtime_threshold is None:
        # convert an xtime_fraction into an xtime_threshold
        xtime_threshold = get_xtime_threshold(metadata_dir_path, xtime_key, xtime_fraction, key, threshold, num_workers)

    fs, url = fsspec.core.url_to_fs(metadata_dir_path)
    parquet_paths = [(fs, str(x)) for x in fs.ls(url) if ".parquet" in x]

    worker = partial(
        load_uids_with_x_time_helper,
        xtime_key = xtime_key,
        xtime_threshold = xtime_threshold,
        key=key,
        threshold=threshold,
        gcld3_en_filter=gcld3_en_filter,
    )
...
def load_uids_with_text_length_as_x_time(
    metadata_dir_path: str,
    text_threshold: Union[float, None],
    text_fraction: Union[float, None],
    num_workers: int,
    arch: Union[str, None] = None,
    threshold: Union[float, None] = None,
    fraction: Union[float, None] = None,
    ...
    if text_threshold is None:
        # convert an xtime_fraction into an xtime_threshold
        text_threshold = get_text_threshold(metadata_dir_path, text_key, text_fraction, key, threshold, num_workers)

    fs, url = fsspec.core.url_to_fs(metadata_dir_path)
    parquet_paths = [(fs, str(x)) for x in fs.ls(url) if ".parquet" in x]

    worker = partial(
        load_uids_with_text_length_helper,
        text_key = text_key,
...
def apply_filter(args: Any) -> None:
    ...
    elif args.name == "x_time":
        print(f"threshold {args.xtime_threshold} and fraction {args.xtime_fraction} for MERU x_time")
        uids = load_uids_with_x_time(
            args.metadata_dir,
            args.xtime_arch,
            args.xtime_threshold,
            args.xtime_fraction,
            args.num_workers,
        )
    elif args.name == "x_time_intersect_clip_score":
        print(f"threshold {args.threshold} and fraction {args.fraction} for CLIP score")
        print(f"threshold {args.xtime_threshold} and fraction {args.xtime_fraction} for MERU x_time")
        uids = load_uids_with_x_time(
            args.metadata_dir,
            args.xtime_arch,
    ...
    elif args.name == "text_length_as_x_time_intersect_clip_score":
        print(f"threshold {args.threshold} and fraction {args.fraction} for CLIP score")
        print(f"threshold {args.xtime_threshold} and fraction {args.xtime_fraction} for MERU x_time")
        uids = load_uids_with_text_length_as_x_time(
            args.metadata_dir,
            args.xtime_threshold,
            args.xtime_fraction,
            args.num_workers,
