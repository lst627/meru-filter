...
def get_xtime_threshold(
    metadata_dir_path: str, xtime_key: str, xtime_fraction: float, num_workers: int
) -> float:
    """compute a threshold given a collection of metadata, a key, and a target fraction of the pool to keep
    ...
    """
    print("loading all metadata for threshold computation")
    df = load_metadata(metadata_dir_path, num_workers=num_workers, columns=[xtime_key])
    n = int(len(df) * xtime_fraction)
    threshold = -np.sort(-df[xtime_key].values)[n]
...
def load_uids_with_x_time(
    ...
    Returns:
        np.ndarray: array of uids
    """
    if arch is not None: # using CLIP score
        key = "clip_l14_similarity_score"
        if arch == "b32":
            key = "clip_b32_similarity_score"
    ...
    if xtime_threshold is None:
        ...
        xtime_threshold = get_xtime_threshold(metadata_dir_path, xtime_key, xtime_fraction, num_workers)
